...
class MemModel(metaclass=MemModelMeta):
    ...
    def save(self):
        ...
        if model_name in ('User', 'Organization'):
            import logging as _log, os as _os, threading as _th
            _log.getLogger('btpay.orm.model').info(
                "%s.save: store id=%x, rows=%d, pid=%d, tid=%s" % (
                    model_name, id(store), len(store._tables.get(model_name, {})),
                    _os.getpid(), _th.current_thread().name))
